...
- 下面是对一些网络编程中常用的函数进行测试：
  ...
  - `bind`函数：
    ...
    ![image-20210618103050470](D:\for_wps\myfiles\typora_imags\image-20210618103050470.png)
...
- 关于TCP发送RST（reset）分节的三种情况的总结：
  ...
  - TCP想取消一个已有的连接，目前遇到了两种这样的情况：
    ...
    - 客户端在服务端的accept函数返回之前断开了连接，会向服务端发送发送一个RST（书中的5.11有说明，下面是书中的图）。
      ...
      ![image-20210702165534818](D:\for_wps\myfiles\typora_imags\image-20210702165534818.png)
...
- 使用`setrlimit`函数给进程设置可以打开的描述符个数（对应练习6.4）
  ...
  ![](D:\for_wps\myfiles\typora_imags\test_f_setrlimit.gif)
...
- 为了方便起见，这里放一个TCP Output的模型（Figure 2.15）：
  ...
  ![image-20210717113545790](D:\for_wps\myfiles\typora_imags\image-20210717113545790.png)

- 为了方便起见，下面放上一个检测各种TCP条件的方法的图（Figure 7.6），**同时也是对`SO_KEEPALIVE`选项的**总结：

  ![image-20210716152939727](D:\for_wps\myfiles\typora_imags\image-20210716152939727.png)
...
- 为了方便起见，下面放上一个`close`函数、`shutdown`函数以及`SO_LINGER`选项的总结（Figure 7.12）：
  ...
  ![image-20210723111256897](D:\for_wps\myfiles\typora_imags\image-20210723111256897.png)
...
- 为了方便起见，附上一个UDP客户端服务端的流程图（Figure 8.1）：
  ...
  ![image-20210801095809595](D:\for_wps\myfiles\typora_imags\image-20210801095809595.png)
...
- 小总结（对应练习8.2）：一般sendto函数的最后一个参数（地址长度len）使用的是recvfrom函数执行过后的len。
  ...
  ![image-20210730220617242](D:\for_wps\myfiles\typora_imags\image-20210730220617242.png)

### 9、Elementary SCTP Sockets

- 为了方便起见，附上一个SCTP一对一的模型（Figure 9.1）：

  ![image-20210801100336105](D:\for_wps\myfiles\typora_imags\image-20210801100336105.png)

- 为了方便起见，附上一个SCTP一对多的模型（Figure 9.2）：

  ![image-20210801100433568](D:\for_wps\myfiles\typora_imags\image-20210801100433568.png)
...
- 如何控制一个网卡的丢包率？
  ...
  ![image-20210806170936714](D:\for_wps\myfiles\typora_imags\image-20210806170936714.png)
...
- 为了方便起见，这里放上一个`addrinfo`结构：
  ...
  ![image-20210811145420303](D:\for_wps\myfiles\typora_imags\image-20210811145420303.png)

  为了方便起见，下面顺便附上一个根据指定的服务名称（可以是十进制端口号）和任何 `ai_socktype` 提示为每个返回的地址返回的 `addrinfo` 结构的数量。

  ![image-20210811145556213](D:\for_wps\myfiles\typora_imags\image-20210811145556213.png)

  为了方便起见，下图总结了我们期望 `getaddrinfo` 如何处理 `IPv4` 和 `IPv6` 地址。 给定前三列中的变量（通过`hints`来指定），“result”列是我们想要返回给调用者的内容。  “Action”列是我们如何获得此结果的方式：

  ![image-20210811170336120](D:\for_wps\myfiles\typora_imags\image-20210811170336120.png)
...
- 关于网关、DNS、子网掩码、MAC地址的简介见博客`http://www.360doc.com/content/19/0424/14/37015604_831140294.shtml`。
  ...
  ![image-20210822113728212](D:\for_wps\myfiles\typora_imags\image-20210822113728212.png)
...
- 将一台`ubuntu18.04`的机器配置成`ftp`服务器：
  ...
  - 在windows上进行验证如下：
    ...
    ![image-20210824161358595](D:\for_wps\myfiles\typora_imags\image-20210824161358595.png)

    输入用户名和密码即可登录，如下：

    ![image-20210824161450550](D:\for_wps\myfiles\typora_imags\image-20210824161450550.png)

    ![image-20210824161500146](D:\for_wps\myfiles\typora_imags\image-20210824161500146.png)

  - 在`ubuntu`上验证，如下：

    ![image-20210824161617669](D:\for_wps\myfiles\typora_imags\image-20210824161617669.png)

- 为了方便起见，放上一个`IPv4` 和 `IPv6` 客户端和服务器之间的互操作性摘要（Figure 12.5）。

  ![image-20210823144211648](D:\for_wps\myfiles\typora_imags\image-20210823144211648.png)
...
- 为了方便起见，下面附上一个`inetd`服务进程的执行流程：
  ...
  ![image-20210825204215150](D:\for_wps\myfiles\typora_imags\image-20210825204215150.png)
  ...
  ![image-20210825204635933](D:\for_wps\myfiles\typora_imags\image-20210825204635933.png)

  连接请求被定向到 TCP 端口 21，但接受创建了一个新的连接套接字。

  下图显示了在调用 `fork` ，子进程关闭了除连接的套接字之外的所有描述符之后。子进程中的描述符：

  ![image-20210825204959896](D:\for_wps\myfiles\typora_imags\image-20210825204959896.png)

  下一步是让孩子将连接的套接字复制到描述符 0、1 和 2，然后关闭连接的套接字，如下图：

  ![image-20210825205126392](D:\for_wps\myfiles\typora_imags\image-20210825205126392.png)
...
- 为了方便起见，下面附上结构体`msghdr`：
  ...
  ![image-20210901142926885](D:\for_wps\myfiles\typora_imags\image-20210901142926885.png)

  下图显示了辅助数据`*msg_control`指向的辅助数据结构示例（Figure 14.12）：

  ![image-20210901144627621](D:\for_wps\myfiles\typora_imags\image-20210901144627621.png)

  ![image-20210907100652618](D:\for_wps\myfiles\typora_imags\image-20210907100652618.png)
  ...
  `CMSG_LEN`和`CMSG_SPACE`的区别在于，前者不计辅助数据对象中数据部分之后可能的填充字节，因而返回的是用于存放在`cmsg_len`成员中的值，后者计上结尾处可能的填充字节，因而返回的是用于为辅助对象动态分配空间的大小值。
  ...
  `cmsgcred`用于Unix域套接字。用来检查`recvmsg`是否成功接收了一个用户凭证可以通过下面的代码进行判断：
  ...
  上面的`(sizeof(struct cmsghdr) + sizeof(struct cmsgcred))`等价于`CMSG_LEN(sizeof(struct cmsgcred))`。

- 为了方便起见，附上一个五组 I/O 功能之间的差异图（Figure 14.10）。

  ![image-20210831210616116](D:\for_wps\myfiles\typora_imags\image-20210831210616116.png)
...
- 下面是自己开发的一个探索自己主机`Ubuntu18.04`的`listen`函数的`backlog`的程序（对应练习15.4），如下：
      ...
      for(backlog = 0; backlog <= 14; backlog++) {
          ...
          if((childpid = fork()) == 0) {                      // 进入子进程
              ...
              if(strcmp(buff, "ready") == 0) {
                  printf("childpid_%d ready\n", childpid);
                  sleep(5);
              } else {
                  printf("childpid_%d error\n", childpid);
                  exit(1);
              }
              for(conn_num = 0; conn_num < 105; conn_num++) {
                  if(alarm(2) != 0) {
                      printf("connect_timeo: alarm was already set\n");
                      exit(1);
                  }
                  pa_sockfd = Socket(AF_INET, SOCK_STREAM, 0);
                  fd_set[fd_cnt++] = pa_sockfd;
                  if((n = connect(pa_sockfd, (SA *) &servaddr, sizeof(servaddr))) < 0 && errno == EINTR) {
                      for(i = 0; i < fd_cnt; i++) {
                          Close(fd_set[i]);
                      }
                      Write(sockfd[0], s, sizeof(s));
                      break;
                  }
                  alarm(0);
              }
          ...
          printf("childpid_%d terminate\n", childpid);
          printf("backlog = %d, connect_num = %d\n", backlog, conn_num);
          printf("\n");
      }
      return 0;
  }
