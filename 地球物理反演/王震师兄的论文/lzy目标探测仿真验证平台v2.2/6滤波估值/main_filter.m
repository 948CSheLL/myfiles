    ...
    NonLine_Pk=P0*eye(6);   % 非线性部分协方差矩阵
     for i=1:length(Plane.v_y)          
        for j=1:length(Plane.v_x)       % 遍历每一个发射线圈的位置
           ...
           [bx,by,bz,Hk]=HFieldModel([v_targetPostion(:) v_inducedDipMoment(:)],RxPostion(1),RxPostion(2),RxPostion(3));    % 计算hk以及Hk（即根据磁矩和位置矢量计算观测场）
           ...
           Z=[real(Z)];
           Hk=[real(Hk)];
           K=NonLine_Pk*Hk'/(Hk*NonLine_Pk*Hk'+R0*eye(length(Z)));    % 卡尔曼增益
           xk=xk+K*(hk-Z);                                  % 更新xk
