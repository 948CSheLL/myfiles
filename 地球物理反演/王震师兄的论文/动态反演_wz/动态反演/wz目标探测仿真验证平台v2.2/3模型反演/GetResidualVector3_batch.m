function [v_Residual,Bs] = GetResidualVector3_batch(detector,Alpha,m_pk,m_data,randdata)
%   残差函数
%   输出: 
...
for i=randdata
    B=LinearizationSecondField(detector,m_pk(i,:),Alpha);
...
%     v_Residual(3*i-2)=abs(B(1)-m_data(i,1));
%     v_Residual(3*i-1)=abs(B(2)-m_data(i,2));
%     v_Residual(3*i)=abs(B(3)-m_data(i,3));
end
