...
function [grad] = GetBsGrad_lw(detector,m_pk,xk,detector_oris)
...
%}
   ...
   %lw u_dt = r1_dt(1); v_dt = r1_dt(2); w_dt = r1_dt(3);
    u_dt = r_dt(1); v_dt = r_dt(2); w_dt = r_dt(3);
    ...
    grad(4,:) = 100 * Gs * M1_grad * Hp; %这里 rt'要不要乘？
    ...
    %这是Hp对x,y,z的偏导
    x_grad_Hp = (1/4)*detector.I*detector.R^2* [3*w_dt*r5 + 3*u_dt*w_dt*x_grad_r5; 
                                                3*v_dt*w_dt*x_grad_r5; 
                                                3*w_dt*w_dt*x_grad_r5 - x_grad_r3];
    y_grad_Hp = (1/4)*detector.I*detector.R^2* [3*u_dt*w_dt*y_grad_r5; 
                                                3*w_dt*r5 + 3*v_dt*w_dt*y_grad_r5; 
                                                3*w_dt*w_dt*y_grad_r5 - y_grad_r3];
    z_grad_Hp = (1/4)*detector.I*detector.R^2* [3*u_dt*r5 + 3*u_dt*w_dt*z_grad_r5; 3*v_dt*r5 + 3*v_dt*w_dt*z_grad_r5; 6*w_dt*r5 +3*w_dt*w_dt*z_grad_r5 - z_grad_r3];
